...
resource "local_file" "hosts_templatefile" {
  content = templatefile("${path.module}/hosts.tftpl",
    {
      sonar_ip = yandex_compute_instance.server["sonar"].network_interface[0].nat_ip_address,
      nexus_ip  = yandex_compute_instance.server["nexus"].network_interface[0].nat_ip_address,
